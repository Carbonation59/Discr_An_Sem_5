...
int main() {
    ...
    std::cout << "Count of lines is " << v.size() << std::endl;

    // Измеряем время работы суффиксного дерева.
    std::chrono::time_point<std::chrono::system_clock> start_ts = std::chrono::system_clock::now();

    for(int i = 0; i < v.size(); i++){
        pattern = v[i] + v[i] + TERMINAL;
    ...
    for(int i = 0; i < v.size(); i++){
        ...
        for(int j = 1; j < v[i].size(); j++){
            string f = tmp[j-1].substr(1) + tmp[j-1][0];
            tmp.push_back(f);
        }
        sort(tmp.begin(), tmp.end(), Conv);
//      cout << tmp[0] << '\n';
    }

    end_ts = std::chrono::system_clock::now();
    uint64_t stl_sort_ts = std::chrono::duration_cast<duration_t>( end_ts - start_ts ).count();
    std::cout << "Suffix Tree time: " << counting_sort_ts << DURATION_PREFIX << std::endl;
    std::cout << "Square algorithm time: " << stl_sort_ts << DURATION_PREFIX << std::endl;
}
